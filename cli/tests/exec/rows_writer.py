...
Run as ``python rows_writer.py <cues> <path> [<path> ...]``. Like the sidecar,
it opens every pipe up front and closes none of them until it has written them
all -- so the process reading the first document sees no end of it while the
second is being written.
"""
