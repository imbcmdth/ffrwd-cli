...
```
-- pin: generated by the compiler when this lands
```
...
rung with each audio rendition; the destination is computed per row,
the same shape recipe 81's `n`-keyed names take, so each pairing lands
in its own file:
...
) TO (:'prefix' || v.height || 'p-' || a.language || '.mp4')
...
Reach for this to produce per-language downloads from a demuxed
ladder - one whose audio rides in its own renditions rather than
muxed into each video variant, since a ladder muxed that way has no
audio-only rows for `a` to pick up.
