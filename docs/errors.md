...
**Meaning:** An input that can only be opened once - a `scheme://` URL, one whose `format =>` names a capture device or a graph, or one the query asked to have paced with `realtime => true` - feeds a query that splits across processes, and the compiler cannot say how deep the buffers between those processes have to be. One process reads such an input and hands every other one a pipe (cookbook recipe 101). Where two of those pipes come back together, the pipe whose path is quicker holds frames until the slower one arrives, and the compiler counts how many. A stage between them that hands on a different number of frames than it reads makes that count meaningless: `fps`, `framerate`, `select`, `decimate`, `trim`, `reverse` and their audio counterparts, or a module whose describe says it is not one frame out per frame in.

**Fires when:** a one-open input feeds two paths that meet again, with such a stage on one of them. Never for a plain file: a file opens as many times as the query needs, so each path decodes it for itself and there is no buffer to size - unless the query paced it with `realtime => true`, which makes it one-open like any other paced source, since `-re` runs off a clock the reading process starts and two such clocks over one program have nothing holding them together. Never for a one-open input read by a single process either - there is nothing to run ahead of.
...
It fires too when a module never listens on the loopback port its feeder is delivered on: the feeder's writer is started only once the port accepts, and after 30 seconds without it the run stops, naming the port and the process that would have written it. And when the run cannot listen on the loopback port a run-time lateral's data stream is written to it on, which was free when the query compiled.
