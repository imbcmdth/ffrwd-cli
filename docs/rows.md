...
**Times are the input's own.** `start_t` and `end_t` are on the clock every other time in a query is on: `ffmpeg.trim(f.video[1], start => v.start_t)` opens on the frame the row describes. That is a promise the compiler keeps rather than something a stream copy gives for free. A stream need not open where its file does - an audio encoder's priming puts the picture behind the sound, a packager writes a timestamp offset, MPEG-TS starts where it likes, a copy cut keeps the packet it cut into - and a copy written without care re-bases the stream on its own first packet and loses that offset, which is exactly what a `trim` is then wrong by. The one time that is not kept is a packet presented BEFORE zero, which a NUT pipe cannot carry: it is reported at zero, where the container itself already starts presenting.
