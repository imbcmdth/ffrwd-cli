...
Subscripts reach track-row columns without unnest: `f.audio[1].tags.language` (strict-Postgres `(f.audio[1]).tags.language` also parses). In a `WHERE` this is an **assertion** - the subscript names one track, so a false predicate refuses to compile ([recipe 29](corpus.md#29-assert-what-youre-shipping)).
...
`WHERE`, `ORDER BY` and `LIMIT` over these columns filter and rank renditions the same as track rows do below ([recipes 105-106](examples.md#105-pick-a-rung-from-an-abr-ladder)); a `format 'hls'`/`format 'dash'` destination fed every surviving rendition writes a new ladder, rung for rung ([recipe 107](corpus.md#107-re-encode-an-abr-ladder-through-to-another-abr-ladder)). Carried through a CTE they stay rows: `r.video[1]` in a body is THAT row's video, one cell per surviving rendition, NULL where the rung carries no track of the kind ([recipe 123](examples.md#123-re-lay-a-muxed-ladder-as-a-demuxed-one)).

`SELECT *` over a rendition alias is `video` then `audio`, one cell per row, NULL where the rung lacks the kind - the same expansion an input alias's star gets, applied per row; `r.*, r.video, r.audio` all mean the same thing and compile to the identical command ([recipes 126-127](corpus.md#126-republish-a-demuxed-ladder-with-select-)). EXCEPT/REPLACE narrow or override those two names, same as over an input's four.

A cell that is an UNMODIFIED read of a rendition row - `r.video[1]`, a bare `r.video`, one carried through WHERE, a CTE, a FULL JOIN, or COALESCE - carries that row's own `name` and `language` into a manifest destination's variant map: a row's name is its video cell's rendition name, or, on an audio-only row, its audio cell's; its language is the audio cell's rendition language, when its stream tags none of its own ([recipe 128](corpus.md#128-a-trimmed-ladder-keeps-its-names)). A CELL a filter built - `scale()`, an encode, `amix()` - is a fresh stream that carries none of this, so its row falls back to the computed name (height for video, language for audio) exactly as before ([recipe 129](corpus.md#129-a-computed-cell-beside-a-copied-one)). A name that cannot be found, or that collides with another - video or audio, since `%v` is one directory namespace regardless of kind - falls back to its position (`v0`, `a1`) either way ([recipe 123](examples.md#123-re-lay-a-muxed-ladder-as-a-demuxed-one) has one: an audio-only row copying its rendition's name off the SAME rung a video row is also named for). `name` rides in HLS's `var_stream_map` only - DASH's `adaptation_sets` carries no names - but `language` reaches a DASH output too, stamped as that stream's own `-metadata` tag.
...
114](corpus.md#114-a-source-whose-rows-are-files-ffmpeg-opens-itself)).
...
**Times are the ones the query's own filters mean.** `start_t` and `end_t` are on the clock every other time in a query is on: `ffmpeg.trim(f.video[1], start => v.start_t)` opens on the frame the row describes. That is a promise the compiler keeps rather than something a stream copy gives for free, and two things would otherwise break it. A stream need not open where its file does - an audio encoder's priming puts the picture behind the sound, a packager writes a timestamp offset, MPEG-TS starts where it likes, a copy cut keeps the packet it cut into - and a copy written without care re-bases the stream on its own first packet and loses that offset. The file's own START is the other: ffmpeg subtracts it before any filter sees a frame, so a time the CONTAINER states is a time a `trim` is wrong by exactly that much. The compile-time read is taken the way the run reads the same stream, so the offset survives and the start is already gone. The one time that is not kept is a packet presented BEFORE zero, which a NUT pipe cannot carry: it is reported at zero, where the graph itself already starts.

The same clock carries every other time a query reads off an input - a [chapter row](#chapter-rows---unnestfchapters-c)'s bounds, a [cue row](#cue-rows---unnestvcues-c)'s - so `WHERE f.t BETWEEN c.start_t AND c.end_t` cuts the span the chapter names however its container numbers it. A `duration` is a length rather than a time, and does not move.

**Writing is the other half.** Rows get INTO a stream's packets through a `RETURNS packets` filter, which the destination places behind the encoder it was already going to place and which takes each producer's rows as an argument of its own - the annotation column a stream module writes, or a rows function's result over one ([dialect.md](dialect.md)). Core ffrwd learns no format either way: what a record means is the module's, and the same package ships the filter that writes and the sinks that read. `ffrwd/index` is that package for embedding vectors. Because the records ride the packets, they survive a `-c copy` remux, a trim and a `UNION ALL`, and the container is free.
...
`WHERE` over row columns filters tracks; `ORDER BY` re-sorts them (multi-key, Postgres NULL placement) - without it, rows keep file order, which is player-visible and never changed implicitly. Both take the compile-time predicate grammar: `=`, `!=`, `<`, `<=`, `>`, `>=`, `BETWEEN`, `IS [NOT] NULL`, `AND`/`OR`/`NOT`, statically type-checked. An operand is a literal, a column of the row, or a value computed over one, on either side - two columns of the same row compare against each other (`WHERE c.end_t > c.start_t`), and two of different relations are a JOIN.
...
Writing is the mirror: an array of `cue` records **in a stream position** IS a WebVTT subtitle track, minted as one self-contained input. Cues must each end after they start and run in ascending order; unlike chapters they may overlap, which WebVTT allows. Because a cue and a chapter are the same shape, converting either way is one `array_agg` - [recipe 65](corpus.md#65-turn-chapters-into-a-subtitle-track-and-back).
...
`start`, `stop`, and `step` must be whole numbers the compiler can count by the time this pass runs, which is after `-v` substitution - `generate_series(1, :count)` is fine, so is `generate_series(1, array_length(ARRAY[1280, 854, 640], 1))` and arithmetic over either, while a column reference or anything else computed at run time is a typed rejection, because that is what keeps the row count (`stop - start` over `step`, inclusive) known before anything runs. A `0` step is rejected, and so is a range that would produce no rows (descending bounds under the default ascending step, or the reverse under a negative one): a series that silently produces nothing is a mistake worth naming, not a valid empty table.
...
- **Over track rows**: the keys land on that row's stream(s) - `-metadata:s:N`. `STRUCT(CASE WHEN t.tags.language IN ('en', 'english') THEN 'eng' ELSE t.tags.language END AS language) AS tags` retags a library in one expression. Keys the column does not name pass through unchanged. Recipes [37-38](corpus.md#37-retitle-tracks-from-their-own-metadata).
- **Over input rows only** (no track rows in the branch): the keys are the container's - `-metadata`. The input's own tags feed the expressions, so `STRUCT(CASE WHEN f.tags.title IS NULL THEN 'Untitled' ELSE f.tags.title END AS title) AS tags` fills a missing title. [Recipe 52](corpus.md#52-read-the-containers-tags-rewrite-them-with-case).
- **Both in one query**: layer with a CTE - a `tags` column in the body is per-stream, in the outer SELECT container-level, and the outer SELECT gathers the CTE's rows (`array_agg` + `GROUP BY`, see Combining rows); the outer value wins on a shared key. [Recipe 53](corpus.md#53-tag-the-tracks-and-the-container-in-one-query).

At container level the map's own SOURCE is written too: `f.tags || STRUCT(...) AS tags` emits `-map_metadata` for that input, and a bare `STRUCT() AS tags` emits `-map_metadata -1`, writing no globals at all. Over track rows a map that names no key is a rejection - there is nothing to set. [Recipe 82](corpus.md#82-keep-a-files-tags-and-change-one).
...
Rows that exist before the run - cues, chapters, a series - are compile-time rows, and a function over one of their columns runs once per row: the built-in text and number functions (`upper(c.text)`), and a value `LANGUAGE wasm` function, memoized on its arguments so two rows saying the same thing cost one call. The result stands wherever a row value does - a `STRUCT(...)::cue` field rebuilding a track, a computed `TO`, a `WHERE` ([recipe 112](corpus.md#112-a-function-over-a-caption-files-cues)).

Rows a module writes during the run are never counted, and a function over them is a **rows function**: `CREATE FUNCTION fauxlate(cues cue[]) RETURNS cue[] AS '<module>', 'fauxlate' LANGUAGE wasm`, one annotation parameter, an annotation return, no stream. Its argument is another module's annotation column (`fauxlate(captions(f.video[1]).cues)`), another rows function's result, or a CTE column bound to either - resolved back to the same producer, so a query that needs the column twice (once on its own, once as the argument) still runs the module once; it runs in that module's sidecar process, fed each row as it is written, and its result is a row column of the declared type - a subtitle track when projected, the rows at a `.ndjson` destination, the document a `RETURNS packets` filter reads when it fills one of that filter's rows arguments. The module says what rows it reads and what it writes; the declaration is checked against both, and the producer's record has to be the one the function reads. A rows function over a file's cues is refused, pointing at the value form ([recipe 113](examples.md#113-translate-captions-as-they-are-produced)). Its own rows are not narrowed on either side of it: the node that narrows rows rides the frames a producer reads them off, so a `WHERE` before the call and a gather over its result are both refused by name.
...
`GROUP BY` and `array_agg` are legal here too - table mode has no destination to fan out over, so every group just prints as one row, in first-appearance order, `array_agg` an array cell of the group's tracks. It is how you preview a fan-out COPY's partitions before writing any file - [recipe 56](corpus.md#56-preview-a-grouped-shape-as-a-table).
