...
A hundred more worked examples, kept whole. The [cookbook](examples.md) is the thirty that teach the language; these are the rest of what it does - the variations, the one-off shapes, the recipes another page links to for a detail. Every command shown here is generated by the compiler and diffed by the same test, so anything on this page works exactly as written.

Entries keep the numbers they have always had, so a link from elsewhere still lands where it meant to.
...
ffmpeg -i tests/fixtures/keys.mkv -filter_complex \
  '[0:v:0]split=2[src_f_v_0_split0][src_f_v_0_split1];'\
'[src_f_v_0_split0]trim=start=3.267:duration=0.2[n1];'\
