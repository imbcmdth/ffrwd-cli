//! A packet sink of the world before `arity` and `input-stream` moved into
//! the shared `types` interface, so the adapter carrying that world is
//! exercised by a module actually shaped that way.
//!
...
impl Guest for Adapted0150 {
    fn describe() -> PacketSinkMeta {
        PacketSinkMeta {
            meta: Meta {
                name: "adapted_0150".to_string(),
