//! `source_replay`, built against the vendored `ffrwd:av@0.15.0` world
//! instead of the current one. Its `open` is told which tracks to pull, the
//! shape a packet source has had since 0.15.0 and still has, so this is the
//! module the host's 0.15.0 arm is proven against: a source built before the
//! 0.16.0 bump still loads, opens and pulls under a 0.16.0 host.
//!
//! It is `source-replay` line for line, its `wit_bindgen::generate!` pointed
//! at the older world instead. The catalog and the seven packets it replays
//! are `source-replay`'s own - see `modules/source-replay/src/lib.rs` for
//! what they are and why they are compiled in rather than read from a file.
