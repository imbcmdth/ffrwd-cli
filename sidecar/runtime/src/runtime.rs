...
//!
//! 0.14.0 adds `rows-module`: no stream at all, JSON rows in and out of one
//! call. It shares nothing with the windowed or packet interfaces, so it is
//! hosted by [`RowsModule`] on its own, the way [`PacketSource`] is.
//!
//! 0.15.0 tells a packet source what to pull: `open` takes the catalog
//! indices the run reads and the source subscribes to those alone. It is the
//! one interface whose shape changed rather than grew, so it is the one that
//! is not adapted - a source built against an older world is refused, naming
//! the world to rebuild against.
//!
//! 0.16.0 adds `packet-filter`: encoded packets in, encoded packets out,
//! with rows arriving beside them. It reads pads the way a packet sink does
//! and hands them on instead of consuming them, so it is hosted by
//! [`PacketFilter`], beside [`PacketSink`]. The same world moves `arity` and
...
//!
//! 0.17.0 adds data streams: `coded-format` gains a `data` arm, so a packet
//! source's catalog and a packet sink's or filter's pads may carry messages,
//! and a sink and a filter say how many data streams they read. No older
...
mod world_0160 {
    ...
    /// carries colorimetry and channel layout, as 0.15.0 does, so it
    ...
    pub mod video {
        wasmtime::component::bindgen!({ path: "../worlds/0.16.0", world: "video-module" });
    }
    pub mod meta {
        wasmtime::component::bindgen!({
            path: "../worlds/0.16.0",
            world: "meta-module",
            with: {
                "ffrwd:av/types": crate::runtime::world_0160::video::ffrwd::av::types,
                "ffrwd:av/filter": crate::runtime::world_0160::video::exports::ffrwd::av::filter,
            },
        });
    }
    pub mod window {
    ...
    pub mod packet {
        wasmtime::component::bindgen!({
            path: "../worlds/0.16.0",
            world: "packet-sink-module",
            with: { "ffrwd:av/types": crate::runtime::world_0160::video::ffrwd::av::types },
        });
    }
    pub mod packet_source {
        wasmtime::component::bindgen!({
    ...
    pub mod values {
        wasmtime::component::bindgen!({ path: "../worlds/0.16.0", world: "values-module" });
    }
    pub mod rows {
        wasmtime::component::bindgen!({
            path: "../worlds/0.16.0",
...
mod world_0150 {
    ...
    /// What this world's `init` is handed, from the host's own format. It
    /// carries colorimetry and channel layout, as 0.14.0 does, so it
    /// converts by hand where the older worlds share a macro.
    pub fn format(format: &crate::runtime::Format) -> video::ffrwd::av::types::Format {
        use video::ffrwd::av::types::{AudioFormat, Format, VideoFormat};
        match format.media {
    ...
    pub mod video {
        wasmtime::component::bindgen!({ path: "../worlds/0.15.0", world: "video-module" });
    }
    pub mod meta {
        wasmtime::component::bindgen!({
            path: "../worlds/0.15.0",
            world: "meta-module",
            with: {
                "ffrwd:av/types": crate::runtime::world_0150::video::ffrwd::av::types,
                "ffrwd:av/filter": crate::runtime::world_0150::video::exports::ffrwd::av::filter,
            },
        });
    }
    pub mod window {
    ...
    pub mod packet {
        wasmtime::component::bindgen!({
            path: "../worlds/0.15.0",
            world: "packet-sink-module",
            with: { "ffrwd:av/types": crate::runtime::world_0150::video::ffrwd::av::types },
        });
    }
    pub mod packet_source {
        wasmtime::component::bindgen!({
    ...
    pub mod values {
        wasmtime::component::bindgen!({ path: "../worlds/0.15.0", world: "values-module" });
    }
    pub mod rows {
        wasmtime::component::bindgen!({
            path: "../worlds/0.15.0",
...
mod world_0140 {
    ...
    /// What this world's `init` is handed, from the host's own format. It
    /// carries colorimetry and channel layout, as 0.13.0 does, so it
    /// converts by hand where the older worlds share a macro.
    pub fn format(format: &crate::runtime::Format) -> video::ffrwd::av::types::Format {
        use video::ffrwd::av::types::{AudioFormat, Format, VideoFormat};
        match format.media {
    ...
    pub mod video {
        wasmtime::component::bindgen!({ path: "../worlds/0.14.0", world: "video-module" });
    }
    pub mod meta {
        wasmtime::component::bindgen!({
            path: "../worlds/0.14.0",
            world: "meta-module",
            with: {
                "ffrwd:av/types": crate::runtime::world_0140::video::ffrwd::av::types,
                "ffrwd:av/filter": crate::runtime::world_0140::video::exports::ffrwd::av::filter,
            },
        });
    }
    pub mod window {
    ...
    pub mod packet {
        wasmtime::component::bindgen!({
            path: "../worlds/0.14.0",
            world: "packet-sink-module",
            with: { "ffrwd:av/types": crate::runtime::world_0140::video::ffrwd::av::types },
        });
    }
    pub mod values {
        wasmtime::component::bindgen!({ path: "../worlds/0.14.0", world: "values-module" });
    }
    pub mod rows {
        wasmtime::component::bindgen!({
            path: "../worlds/0.14.0",
...
mod world_0130 {
    ...
    pub mod video {
        wasmtime::component::bindgen!({ path: "../worlds/0.13.0", world: "video-module" });
    }
    pub mod meta {
        wasmtime::component::bindgen!({
            path: "../worlds/0.13.0",
    ...
    pub mod values {
        wasmtime::component::bindgen!({ path: "../worlds/0.13.0", world: "values-module" });
    }
}
...
pub enum CodedFormat {
    ...
    /// A data stream: each packet one message at its own pts, every one a
    /// keyframe, and no geometry. Carried from 0.17.0 on.
    Data,
}

impl CodedFormat {
    /// `video`, `audio` or `data`, for a message.
    pub fn kind(&self) -> &'static str {
        match self {
            CodedFormat::Video { .. } => "video",
            CodedFormat::Audio { .. } => "audio",
            CodedFormat::Data => "data",
        }
    }
}
...
/// A world's own `arity` in the host's spelling, for a world from 0.16.0 on,
/// where it lives in the shared `types` interface.
macro_rules! arity_from_wit {
    ($world:ident, $arity:expr) => {{
        use $world::video::ffrwd::av::types::Arity as Wit;
        match $arity {
            Wit::Zero => Arity::Zero,
            Wit::One => Arity::One,
            Wit::Many => Arity::Many,
...
pub struct DescribedPacketSink {
    ...
    pub audio: Arity,
    /// How many data streams the sink reads. `Zero` for a world before
    /// 0.17.0, which carried none.
    pub data: Arity,
...
pub struct Described {
    ...
    pub inputs: u32,
    /// The call's arguments the module reads itself over a loopback
    /// connection rather than as pads. Declared by a windowed module from
    /// 0.17.0 on; empty for every other.
    pub feeders: Vec<Feeder>,
}

/// One feeder argument of a windowed module: a stream the module reads
/// itself, as NUT over a loopback connection, rather than being handed its
/// frames.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Feeder {
    /// Which argument of the call it is, counting stream arguments from 0.
    pub input: u32,
    /// The param the host writes the port it picked into.
    pub port_param: String,
    /// `video` or `audio`.
    pub kind: String,
    /// Feeders sharing one connection; empty is a connection of its own.
    pub group: String,
}
...
fn link(
    ...
    .map_err(wasm_err)?;
    world_0160::window::ffrwd::av::window_source::add_to_linker::<_, HasSelf<_>>(
        &mut linker,
        |host: &mut Host| host,
    )
    .map_err(wasm_err)?;
    world_0150::window::ffrwd::av::window_source::add_to_linker::<_, HasSelf<_>>(
        &mut linker,
        |host: &mut Host| host,
    )
    .map_err(wasm_err)?;
    world_0140::window::ffrwd::av::window_source::add_to_linker::<_, HasSelf<_>>(
        &mut linker,
        |host: &mut Host| host,
    )
    .map_err(wasm_err)?;
...
video_adapter!(Video0140, world_0140, "0.14.0");
...
meta_adapter!(Meta0140, world_0140, "0.14.0");
...
/// what `kind_bearing_window_adapter!` does. `$feeders` reads the feeder
/// arguments off the description, for a world that has the field.
macro_rules! borrowed_window_adapter {
    ($name:ident, $world:ident, $resolve:ident, $version:literal, $feeders:expr) => {
...
borrowed_window_adapter!(
    ...
    resolve_0170,
    "0.17.0",
    |d: &world_0170::window::exports::ffrwd::av::window_filter::WindowMeta| d
        .feeders
        .iter()
        .map(|f| Feeder {
            input: f.input,
...
borrowed_window_adapter!(
    ...
    resolve_0160,
    "0.16.0",
    |_: &world_0160::window::exports::ffrwd::av::window_filter::WindowMeta| Vec::new()
);
borrowed_window_adapter!(
    Window0150,
    world_0150,
    resolve_0150,
    "0.15.0",
    |_: &world_0150::window::exports::ffrwd::av::window_filter::WindowMeta| Vec::new()
);
borrowed_window_adapter!(
    Window0140,
    world_0140,
    resolve_0140,
    "0.14.0",
    |_: &world_0140::window::exports::ffrwd::av::window_filter::WindowMeta| Vec::new()
);
borrowed_window_adapter!(
    Window0130,
    world_0130,
    resolve_0130,
    "0.13.0",
    |_: &world_0130::window::exports::ffrwd::av::window_filter::WindowMeta| Vec::new()
);
borrowed_window_adapter!(
    Window0120,
    world_0120,
    resolve_0120,
    "0.12.0",
    |_: &world_0120::window::exports::ffrwd::av::window_filter::WindowMeta| Vec::new()
);
borrowed_window_adapter!(
    Window0110,
    world_0110,
    resolve_0110,
    "0.11.0",
    |_: &world_0110::window::exports::ffrwd::av::window_filter::WindowMeta| Vec::new()
);
...
resolve_frames!(resolve_0140, world_0140);
...
fn instantiate(module_path: &str, purpose: Purpose) -> Result<Opened> {
    ...
    let instance: Box<dyn Adapter> =
        ...
        } else if has_export(&component, &interface("window-filter", "0.16.0")) {
            Box::new(Window0160(
                world_0160::window::WindowModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("window-filter", "0.15.0")) {
            Box::new(Window0150(
                world_0150::window::WindowModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("window-filter", "0.14.0")) {
            Box::new(Window0140(
                world_0140::window::WindowModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("window-filter", "0.13.0")) {
            Box::new(Window0130(
        ...
        } else if has_export(&component, &interface("meta-filter", "0.17.0")) {
            Box::new(Meta0170(
                world_0170::meta::MetaModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("meta-filter", "0.16.0")) {
            Box::new(Meta0160(
                world_0160::meta::MetaModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("meta-filter", "0.15.0")) {
            Box::new(Meta0150(
                world_0150::meta::MetaModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("meta-filter", "0.14.0")) {
            Box::new(Meta0140(
                world_0140::meta::MetaModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        ...
        } else if has_export(&component, &interface("filter", "0.17.0")) {
            Box::new(Video0170(
                world_0170::video::VideoModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("filter", "0.16.0")) {
            Box::new(Video0160(
                world_0160::video::VideoModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("filter", "0.15.0")) {
            Box::new(Video0150(
                world_0150::video::VideoModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
        } else if has_export(&component, &interface("filter", "0.14.0")) {
            Box::new(Video0140(
                world_0140::video::VideoModule::instantiate(&mut store, &component, &linker)
                    .map_err(wasm_err)
                    .with_context(context)?,
            ))
...
enum ValuesInstance {
    ...
    W0140(world_0140::values::ValuesModule),
...
impl ValuesInstance {
    fn list_functions(&self, store: &mut Store<Host>) -> Result<Vec<FunctionMeta>> {
        ...
        Ok(match self {
            ...
            ValuesInstance::W0140(b) => listed!(b),
    ...
    fn invoke(
        ...
        match self {
            ...
            ValuesInstance::W0140(b) => b.ffrwd_av_values().call_invoke(store, name, args),
...
fn instantiate_values(
    ...
    } else if has_export(&component, &interface("values", "0.16.0")) {
        ValuesInstance::W0160(
            world_0160::values::ValuesModule::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("values", "0.15.0")) {
        ValuesInstance::W0150(
            world_0150::values::ValuesModule::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("values", "0.14.0")) {
        ValuesInstance::W0140(
            world_0140::values::ValuesModule::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("values", "0.13.0")) {
...
fn no_data_stream(name: &str, world: &str) -> anyhow::Error {
    anyhow!(
        "{name} is built against ffrwd:av@{world}, which carries no data stream; rebuild it \
         against {WORLD_PACKAGE} to hand it one"
    )
}
...
enum PacketInstance {
    ...
    W0140(world_0140::packet::PacketSinkModule),
...
impl PacketInstance {
    fn describe(&self, store: &mut Store<Host>) -> Result<DescribedPacketSink> {
        ...
        // 0.16.0 on says how much of the stream it needs.
        ...
        // 0.17.0 on says how many data streams it reads.
        macro_rules! with_data {
            ($b:expr, $world:ident, $version:literal) => {{
                let mut described = with_wants!($b, $world, $version);
                let d = $b
                    .ffrwd_av_packet_sink()
                    .call_describe(&mut *store)
                    .map_err(wasm_err)?;
                described.data = arity_from_wit!($world, d.data);
                described
            }};
        }
        Ok(match self {
    ...
    fn init(
        ...
        macro_rules! one_video_stream {
            ($b:expr, $world:ident) => {{
                ...
                let CodedFormat::Video { width, height, .. } = input.stream.format else {
                    bail!(
                        "{name} reads one video stream, and pad 0 carries {}",
        ...
        // still carries them on `packet-sink` itself. `$data` is what a data
        // pad becomes: the world's own arm from 0.17.0 on, and a refusal
        // before it, though `check_sink_inputs` has refused one already.
        macro_rules! several_streams {
            ($b:expr, $world:ident, $($coded:tt)::+, $data:expr, $push_extra:expr) => {{
                use $($coded)::+::CodedStream as WitCodedStream;
        ...
        match self {
            PacketInstance::W0170(b) => several_streams!(
                b,
                world_0170,
                world_0170::video::ffrwd::av::types,
                world_0170::video::ffrwd::av::types::CodedFormat::Data,
                |coded, info, row, rendition, decode_delay| {
                    world_0170::packet::exports::ffrwd::av::packet_sink::InputStream {
                        coded,
                        info,
                        row,
                        rendition: world_0170::rendition_meta(rendition),
                        decode_delay,
                    }
                }
            ),
            PacketInstance::W0160(b) => several_streams!(
                b,
                world_0160,
                world_0160::video::ffrwd::av::types,
                return Err(no_data_stream(name, "0.16.0")),
                |coded, info, row, rendition, decode_delay| {
            ...
            PacketInstance::W0150(b) => several_streams!(
                b,
                world_0150,
                world_0150::video::ffrwd::av::types,
                return Err(no_data_stream(name, "0.15.0")),
                ...
                |coded, info, row, rendition, _delay| {
                    world_0150::packet::exports::ffrwd::av::packet_sink::InputStream {
                        ...
                        info,
                        row,
                        rendition: world_0150::rendition_meta(rendition),
                    }
                }
            ),
            PacketInstance::W0140(b) => several_streams!(
                b,
                world_0140,
                world_0140::video::ffrwd::av::types,
                return Err(no_data_stream(name, "0.14.0")),
                // The reorder depth arrived in 0.16.0; this world's
                // `input-stream` has no field for it, so it is dropped.
                |coded, info, row, rendition, _delay| {
                    world_0140::packet::exports::ffrwd::av::packet_sink::InputStream {
                        ...
                        info,
                        row,
                        rendition: world_0140::rendition_meta(rendition),
                    }
                }
            ),
            PacketInstance::W0130(b) => several_streams!(
                ...
                return Err(no_data_stream(name, "0.13.0")),
                // The reorder depth arrived in 0.16.0; this world's
                // `input-stream` has no field for it, so it is dropped.
                |coded, info, row, rendition, _delay| {
            ...
            PacketInstance::W0120(b) => several_streams!(
                ...
                return Err(no_data_stream(name, "0.12.0")),
    ...
    fn set_params(&self, store: &mut Store<Host>, params: &str) -> Result<Result<(), String>> {
        match self {
            ...
            PacketInstance::W0140(b) => b.ffrwd_av_packet_sink().call_set_params(store, params),
    ...
    fn process(
        ...
        Ok(match self {
            ...
            PacketInstance::W0160(b) => {
                several_pads!(b, world_0160, world_0160::video::ffrwd::av::types)
            }
            PacketInstance::W0150(b) => {
                several_pads!(b, world_0150, world_0150::video::ffrwd::av::types)
            }
            PacketInstance::W0140(b) => {
                several_pads!(b, world_0140, world_0140::video::ffrwd::av::types)
            }
...
fn instantiate_packet(
    ...
    } else if has_export(&component, &interface("packet-sink", "0.16.0")) {
        PacketInstance::W0160(
            world_0160::packet::PacketSinkModule::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("packet-sink", "0.15.0")) {
        PacketInstance::W0150(
            world_0150::packet::PacketSinkModule::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("packet-sink", "0.14.0")) {
        PacketInstance::W0140(
            world_0140::packet::PacketSinkModule::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("packet-sink", "0.13.0")) {
...
fn check_sink_inputs(described: &DescribedPacketSink, inputs: &[SinkInput]) -> Result<()> {
    ...
    if !DATA_WORLDS.contains(&described.world)
        && inputs.iter().any(|i| i.stream.format == CodedFormat::Data)
    {
        return Err(no_data_stream(name, described.world));
    }
    // No codec list names a data stream's codec: "json" is the only one the
    // wire carries, and every module reading data reads it.
    let any_codec = Vec::new();
    for (kind, arity, accepted) in [
        ("video", described.video, &described.video_codecs),
        ("audio", described.audio, &described.audio_codecs),
        ("data", described.data, &any_codec),
    ] {
...
/// What a packet filter publishes without being opened for any stream: a
/// sink's declaration plus whether it acts on the rows handed to `process`.
#[derive(Debug, Clone)]
pub struct DescribedPacketFilter {
    pub meta: Meta,
    /// ffmpeg names for the VIDEO codecs accepted, most preferred first;
    /// empty is every codec.
    pub video_codecs: Vec<String>,
    /// The same for AUDIO.
    pub audio_codecs: Vec<String>,
    /// How many video streams the filter reads.
    pub video: Arity,
    /// How many audio streams the filter reads.
    pub audio: Arity,
    /// How many data streams the filter reads. `Zero` for a world before
    /// 0.17.0, which carried none.
    pub data: Arity,
    /// Whether the filter acts on the rows it is handed, rather than merely
    /// receiving them.
    pub reads_rows: bool,
    /// The wit package version the module was built against.
    pub world: &'static str,
...
/// What one packet-filter call produced: packets per pad, plus rows.
#[derive(Debug, Clone, Default)]
pub struct Filtered {
    /// The packets leaving, one list per pad in `open`'s order.
    pub pads: Vec<Vec<Packet>>,
    pub rows: Vec<String>,
    /// Rows with no packet left to prompt them; only the final call may
    /// carry any.
    pub trailing: Vec<String>,
...
/// Whether the component at `module_path` exports the packet-filter
/// interface, and so transforms encoded packets rather than consuming them.
/// The interface arrived in 0.16.0, so no earlier world answers.
pub fn exports_packet_filter(module_path: &str) -> Result<bool> {
    let component = compile(module_path)?;
    Ok(world_exporting(&component, "packet-filter").is_some())
}

/// Errors naming the component's actual exports when the packet-filter
/// interface is missing from every world. The interface arrived in 0.16.0,
/// and every world carrying it is hosted.
fn check_packet_filter_export(component: &Component, module_path: &str) -> Result<()> {
    if world_exporting(component, "packet-filter").is_some() {
        return Ok(());
    }
    let wanted = interface("packet-filter", WORLD);
    let exports = component_exports(component);
    if exports.is_empty() {
        bail!("{module_path} exports nothing, so not {wanted}");
    }
...
impl PacketFilterInstance {
    /// The filter's own `describe()`, read before it is opened for any
    /// stream.
    fn describe(&self, store: &mut Store<Host>) -> Result<DescribedPacketFilter> {
        macro_rules! described {
            ($b:expr, $world:ident, $version:literal) => {{
                let d = $b
                    ...
                    .call_describe(&mut *store)
                    .map_err(wasm_err)?;
                DescribedPacketFilter {
                    meta: $world::meta(d.meta),
                    video_codecs: d.video_codecs,
                    audio_codecs: d.audio_codecs,
                    video: arity_from_wit!($world, d.video),
                    audio: arity_from_wit!($world, d.audio),
                    // A world with no field for it reads no data stream.
                    data: Arity::Zero,
                    reads_rows: d.reads_rows,
                    world: $version,
                }
        ...
        Ok(match self {
            PacketFilterInstance::W0170(b) => {
                ...
                let d = b
                    ...
                    .call_describe(&mut *store)
                    .map_err(wasm_err)?;
                described.data = arity_from_wit!(world_0170, d.data);
                described
            }
            PacketFilterInstance::W0160(b) => described!(b, world_0160, "0.16.0"),
        })
    }

    /// `init`, with every pad in the world's own spelling, and its answer -
    /// the streams leaving - back in the host's.
    fn init(
        &self,
        store: &mut Store<Host>,
        inputs: &[SinkInput],
        name: &str,
        params: &str,
    ) -> Result<Result<Vec<CodedStream>, String>> {
        macro_rules! opened {
            ($b:expr, $world:ident, $conv:ident) => {{
                use $world::packet_filter::exports::ffrwd::av::packet_filter::InputStream;
                let mut streams = Vec::with_capacity(inputs.len());
                for input in inputs {
                    streams.push(InputStream {
                        coded: $conv::coded_stream_to_wit(&input.stream, name)?,
                        info: $world::stream_info(&input.info, input.stream.time_base, name)?,
                        row: input.row,
                        rendition: $world::rendition_meta(input.rendition.clone()),
                    ...
                    .map_err(wasm_err)?
                {
                    Ok(answered) => Ok(answered
                        .into_iter()
                        .map(|coded| $conv::coded_stream_from_wit(coded, name))
    ...
    fn process(
        ...
        macro_rules! filtered {
            ($b:expr, $world:ident) => {{
                use $world::video::ffrwd::av::types::{Packet as WitPacket, PadPackets as WitPads};
                let carried: Vec<WitPads> = pads
                    .iter()
                    .map(|packets| WitPads {
                        packets: packets
                            .iter()
                ...
                let produced = $b
                    .ffrwd_av_packet_filter()
                    .call_process(&mut *store, &carried, rows, last)
                    .map_err(wasm_err)?;
                Filtered {
                    pads: produced
                        .pads
...
/// Compiles and instantiates the component at `module_path` against the
/// packet-filter world it was built for. Shared by `describe_packet_filter`
/// and `PacketFilter::open`.
fn instantiate_packet_filter(
    module_path: &str,
    purpose: Purpose,
) -> Result<(Store<Host>, PacketFilterInstance)> {
    let component = compile(module_path)?;
    ...
    let instance = if has_export(&component, &interface("packet-filter", "0.17.0")) {
        ...
        PacketFilterInstance::W0160(
            world_0160::packet_filter::PacketFilterModule::instantiate(
                &mut store, &component, &linker,
            )
            .map_err(wasm_err)
            .with_context(context)?,
...
/// Compiles and instantiates the component at `module_path` far enough to
/// call the filter's `describe()`, without opening it for any stream.
pub fn describe_packet_filter(module_path: &str) -> Result<DescribedPacketFilter> {
    let (mut store, instance) = instantiate_packet_filter(module_path, Purpose::Describe)?;
    instance.describe(&mut store)
}

/// One instantiated packet filter: encoded packets and rows in, encoded
/// packets and rows out. Single-threaded by contract, like [`PacketSink`].
pub struct PacketFilter {
    store: Store<Host>,
    instance: PacketFilterInstance,
    meta: Meta,
    /// Whether the filter acts on the rows it is handed: a host with none to
    /// give refuses one that does rather than run it blind.
    reads_rows: bool,
    /// The streams leaving, as `init` answered them: one per pad, in pad
    /// order. The host writes its output headers from these.
    streams: Vec<CodedStream>,
    /// Whether the final call has been made, which may happen once.
    finished: bool,
}

/// The streams a filter was handed against the shape it declared, reusing
/// the sink's own check: the two declare the same thing.
fn check_filter_inputs(described: &DescribedPacketFilter, inputs: &[SinkInput]) -> Result<()> {
    check_sink_inputs(
        &DescribedPacketSink {
            meta: described.meta.clone(),
            video_codecs: described.video_codecs.clone(),
...
impl PacketFilter {
    /// Compiles (cached process-wide by path) and instantiates the component
    /// at `module_path`, then calls the filter's `init` with every pad it
    /// reads: video pads first, then audio, then data, each group in query
    /// order. The count of each kind must be a shape the filter declared, and
    /// every stream's codec one it accepts.
    ///
    ...
    pub fn open(module_path: &str, inputs: &[SinkInput], params: &str) -> Result<PacketFilter> {
        ...
        let described = instance.describe(&mut store)?;
        let meta = described.meta.clone();
        check_filter_inputs(&described, inputs)?;
        ...
        for (pad, (stream, input)) in answered.into_iter().zip(inputs).enumerate() {
            if stream.codec != input.stream.codec {
                bail!(
                    "{}: pad {pad} carries {} and the filter answered {}; a filter hands on the \
                     stream it was given",
                    meta.name,
                    input.stream.codec,
                    stream.codec
            ...
            if stream.time_base != input.stream.time_base {
                bail!(
                    "{}: pad {pad} counts time in {}/{} and the filter answered {}/{}; the time \
                     base is not a filter's to change",
                    meta.name,
                    input.stream.time_base.num,
                    input.stream.time_base.den,
                    stream.time_base.num,
                    stream.time_base.den
                );
            }
            if stream.format != input.stream.format {
                bail!(
                    "{}: pad {pad} carries {} and the filter answered a different geometry; \
        ...
        Ok(PacketFilter {
            ...
            instance,
            meta,
            reads_rows: described.reads_rows,
            streams,
            finished: false,
        })
    }

    /// The streams leaving, one per pad in `open`'s order: what the host
    /// writes its output headers from.
    pub fn streams(&self) -> &[CodedStream] {
        &self.streams
    }

    /// Whether this filter acts on the rows it is handed.
    pub fn reads_rows(&self) -> bool {
        self.reads_rows
    }

    /// How many pads this filter was opened for.
    pub fn pads(&self) -> usize {
        self.streams.len()
    }

    /// The filter's `describe()` meta, read once at open.
    pub fn meta(&self) -> &Meta {
        &self.meta
    }
    ...
    /// Replaces the filter's parameters between calls.
    pub fn set_params(&mut self, params: &str) -> Result<()> {
        self.instance
            .set_params(&mut self.store, params)?
            .map_err(|e| anyhow!("{} rejected params: {e}", self.meta.name))?;
    ...
    /// Packets and rows through the filter: one packet list per pad in the
    /// order `open` was given them, each in decode order, plus whatever rows
    /// have arrived since the last call. `last` marks the final call, which
    /// may carry nothing at all and happens once; everything the filter held
    /// back leaves on it.
    pub fn process(
        &mut self,
        pads: &[Vec<Packet>],
        ...
        let produced = self.instance.process(&mut self.store, pads, rows, last)?;
        if produced.pads.len() != self.streams.len() {
            bail!(
                "{}: opened for {} pad(s) and answered {}",
                self.meta.name,
                self.streams.len(),
                produced.pads.len()
            );
...
// The host's own format into the wire's and back. The coded shapes a packet
// source and a packet filter read have not changed since 0.15.0 but for the
// data arm 0.17.0 added, so one macro generates every world's conversions,
// handed that arm in each direction: the world's own from 0.17.0 on, and
...
macro_rules! coded_conversions {
    ($module:ident, $world:ident, $version:literal, { $($from_data:tt)* }, { $($to_data:tt)* }) => {
        mod $module {
            ...
            /// The host's own coded stream in this world's spelling. A data
            /// stream reaching a world with no arm for it is refused naming
            /// the module and the world.
            // A world whose only coded interface is a packet source never
            // hands a stream to a module, so there this goes unused.
            #[allow(dead_code)]
            pub fn coded_stream_to_wit(
                stream: &CodedStream,
                name: &str,
            ) -> Result<$world::video::ffrwd::av::types::CodedStream> {
                use $world::video::ffrwd::av::types as wit;
                if stream.format == CodedFormat::Data && !DATA_WORLDS.contains(&$version) {
                    return Err(no_data_stream(name, $version));
                }
                ...
                let format = match stream.format {
                    CodedFormat::Video {
                        ...
                        sample_aspect_ratio: sample_aspect_ratio
                            .map(|(num, den)| wit::Rational { num, den }),
                        color: color.map($world::color_info),
                    }),
                    CodedFormat::Audio {
                        sample_rate,
                ...
                Ok(wit::CodedStream {
                    codec: stream.codec.clone(),
                    time_base: wit::Rational { num, den },
                    format,
...
/// against when that world is one this host does not drive.
fn check_packet_source_export(component: &Component, module_path: &str) -> Result<()> {
    match world_exporting(component, "packet-source") {
        Some(world) if PACKET_SOURCE_WORLDS.contains(&world) => return Ok(()),
        Some(world) => bail!(
            "{module_path} exports {}, whose open is not told which tracks to pull; \
             rebuild it against {WORLD_PACKAGE}",
            interface("packet-source", world)
        ),
        None => {}
    }
...
/// The interface has not changed shape since 0.15.0 but for the data arm of
...
impl PacketSourceInstance {
    /// The source's own `describe()`, and the world it was built against.
    ...
    fn next(&self, store: &mut Store<Host>, name: &str) -> Result<Option<Vec<PadPackets>>> {
        macro_rules! pulled {
            ($b:expr, $conv:ident) => {{
                ...
                produced.map($conv::pad_packets_from_wit)
            }};
        }
        Ok(match self {
...
fn instantiate_packet_source(
    ...
    let instance = if has_export(&component, &interface("packet-source", "0.17.0")) {
    ...
    } else if has_export(&component, &interface("packet-source", "0.16.0")) {
        PacketSourceInstance::W0160(
            world_0160::packet_source::PacketSourceModule::instantiate(
                &mut store, &component, &linker,
            )
            .map_err(wasm_err)
            .with_context(context)?,
        ...
        PacketSourceInstance::W0150(
            world_0150::packet_source::PacketSourceModule::instantiate(
                &mut store, &component, &linker,
            )
            .map_err(wasm_err)
            .with_context(context)?,
...
impl PacketSource {
    ...
    pub fn probe(module_path: &str, params: &str) -> Result<Catalog> {
        let (mut store, instance) = instantiate_packet_source(module_path, Purpose::Describe)?;
        let name = instance.describe(&mut store)?.0.name;
        instance.probe(&mut store, params, &name)
    }

    /// Compiles, instantiates and opens the component at `module_path` for a
    /// run, subscribing to `tracks` alone - indices into the catalog `probe`
    /// read at compile time, in the order the caller wants them. The catalog
    /// returned is that restriction, one track per index in that order, and
    /// a source answering any other number of tracks is refused.
    pub fn open(
        module_path: &str,
        params: &str,
        tracks: &[u32],
    ) -> Result<(PacketSource, Catalog)> {
        let (mut store, instance) = instantiate_packet_source(module_path, Purpose::Run)?;
        let meta = instance.describe(&mut store)?.0;
        let catalog = instance.open(&mut store, params, tracks, &meta.name)?;
        if catalog.tracks.len() != tracks.len() {
            bail!(
                "{}: opened for {} track(s) and answered a catalog of {}",
                meta.name,
                tracks.len(),
                catalog.tracks.len()
            );
    ...
    /// One pull: a packet list per track `open` was given, in that order, or
    /// none once the source has nothing left. Calling again after none is
    /// refused, since that answer may only come once.
    ...
    pub fn next(&mut self) -> Result<Option<Vec<PadPackets>>> {
        ...
        let produced = self.instance.next(&mut self.store, &self.meta.name)?;
...
/// What a rows module publishes: `meta`, plus the schema of the row it reads
/// on `process`. `meta`'s own `rows-schema` is what it emits.
#[derive(Debug, Clone)]
pub struct DescribedRowsModule {
    pub meta: Meta,
    pub input_rows_schema: String,
}

/// Whether the component at `module_path` exports the rows-module interface:
/// no stream at all, JSON rows in and out of one call. The interface arrived
/// in 0.14.0, so no earlier world answers.
pub fn exports_rows_module(module_path: &str) -> Result<bool> {
    let component = compile(module_path)?;
    Ok(world_exporting(&component, "rows-module").is_some())
}

/// Errors naming the component's actual exports when the rows-module
/// interface is missing from every world.
fn check_rows_module_export(component: &Component, module_path: &str) -> Result<()> {
    if world_exporting(component, "rows-module").is_some() {
        return Ok(());
    }
    let wanted = interface("rows-module", WORLD);
    let exports = component_exports(component);
    if exports.is_empty() {
        bail!("{module_path} exports nothing, so not {wanted}");
    }
...
/// One instantiated rows module, in whichever world it was built against.
...
enum RowsInstance {
    ...
    W0140(world_0140::rows::RowsModuleHost),
}
...
impl RowsInstance {
        ...
        Ok(match self {
            RowsInstance::W0170(b) => {
                let d = b
                    ...
                    .map_err(wasm_err)?;
                DescribedRowsModule {
                    meta: world_0170::meta(d.meta),
                    input_rows_schema: d.input_rows_schema,
                }
            }
            RowsInstance::W0160(b) => {
                let d = b
                    .ffrwd_av_rows_module()
                    .call_describe(&mut *store)
                    .map_err(wasm_err)?;
                DescribedRowsModule {
                    meta: world_0160::meta(d.meta),
                    input_rows_schema: d.input_rows_schema,
                }
            }
            RowsInstance::W0150(b) => {
                let d = b
                    .ffrwd_av_rows_module()
                    .call_describe(&mut *store)
                    .map_err(wasm_err)?;
                DescribedRowsModule {
                    meta: world_0150::meta(d.meta),
                    input_rows_schema: d.input_rows_schema,
                }
            }
            RowsInstance::W0140(b) => {
                let d = b
                    .ffrwd_av_rows_module()
                    .call_describe(&mut *store)
                    .map_err(wasm_err)?;
                DescribedRowsModule {
                    meta: world_0140::meta(d.meta),
                    input_rows_schema: d.input_rows_schema,
                }
            }
        })
    ...
    fn init(&self, store: &mut Store<Host>, params: &str) -> Result<Result<(), String>> {
        match self {
            ...
            RowsInstance::W0140(b) => b.ffrwd_av_rows_module().call_init(store, params),
        }
        .map_err(wasm_err)
    }
    ...
    fn process(
        ...
        match self {
            ...
            RowsInstance::W0140(b) => b.ffrwd_av_rows_module().call_process(store, rows),
        }
        .map_err(wasm_err)
    }
    ...
    fn finish(&self, store: &mut Store<Host>) -> Result<Result<Vec<String>, String>> {
        match self {
            ...
            RowsInstance::W0140(b) => b.ffrwd_av_rows_module().call_finish(store),
        }
        .map_err(wasm_err)
    }
}
...
/// Compiles and instantiates the component at `module_path` against the
/// rows-module world it was built for. Shared by `describe_rows_module` and
/// `RowsModule::open`.
fn instantiate_rows_module(
    module_path: &str,
    purpose: Purpose,
) -> Result<(Store<Host>, RowsInstance)> {
    let component = compile(module_path)?;
    ...
    } else if has_export(&component, &interface("rows-module", "0.16.0")) {
        RowsInstance::W0160(
            world_0160::rows::RowsModuleHost::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else if has_export(&component, &interface("rows-module", "0.15.0")) {
        RowsInstance::W0150(
            world_0150::rows::RowsModuleHost::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    } else {
        RowsInstance::W0140(
            world_0140::rows::RowsModuleHost::instantiate(&mut store, &component, &linker)
                .map_err(wasm_err)
                .with_context(context)?,
        )
    };
...
/// Compiles and instantiates the component at `module_path` far enough to
/// call the rows module's `describe()`, without `init`-ing it.
pub fn describe_rows_module(module_path: &str) -> Result<DescribedRowsModule> {
    let (mut store, instance) = instantiate_rows_module(module_path, Purpose::Describe)?;
    instance.describe(&mut store)
}

/// One instantiated rows module: no stream at all, JSON rows in and out of
/// one call. Single-threaded by contract, like [`PacketSink`].
pub struct RowsModule {
    store: Store<Host>,
    instance: RowsInstance,
    meta: Meta,
    /// Whether `finish` has been called, which may happen once and only
    /// after the last `process`.
    finished: bool,
}

impl RowsModule {
    /// Compiles (cached process-wide by path), instantiates and `init`s the
    /// component at `module_path`.
    pub fn open(module_path: &str, params: &str) -> Result<RowsModule> {
        let (mut store, instance) = instantiate_rows_module(module_path, Purpose::Run)?;
    ...
    /// JSON rows in, JSON rows out. May be called any number of times before
    /// `finish`.
    pub fn process(&mut self, rows: &[String]) -> Result<Vec<String>> {
        if self.finished {
            bail!(
                "{}: process called after finish, which is the last call",
                self.meta.name
            );
        }
        self.instance
            .process(&mut self.store, rows)?
            .map_err(|e| anyhow!("{}: {e}", self.meta.name))
    }

    /// Whatever the module held back across every `process` call. Called
    /// once, after the last `process`.
    pub fn finish(&mut self) -> Result<Vec<String>> {
        if self.finished {
            bail!(
                "{}: finish called again, which happens once",
                self.meta.name
            );
        }
        self.finished = true;
        self.instance
            .finish(&mut self.store)?
            .map_err(|e| anyhow!("{}: {e}", self.meta.name))
    }
}
